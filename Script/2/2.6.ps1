...
if($row['value_configured'] -eq 0 -and $row['value_in_use'] -eq 0){
    Write-Output "No changes needed. 'Remote Access' Server Configuration Option is set to 0."
}
else{
    ...
    if($sqlDataAdapter){
        Write-Output "Successfully configured 'Remote Access' Server Configuration Option is set to 0."
